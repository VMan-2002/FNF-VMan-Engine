...
class Paths {
	...
	static var webpImages:Map<String, Bool> = new Map<String, Bool>();
	...
	//todo: webp loading
	static public function imageNew(key:String, ?library:String) {
		inline function webpThing() {
			var path = getPath('images/${key}.webp', IMAGE, library);
		}
		inline function pngThing() {
			var path = getPath('images/${key}.png', IMAGE, library);
		}
		if (!webpImages.exists(key)) {
			if (OpenFlAssets.exists(key + ".webp")) {
				webpImages.set(key, true);
				webpThing();
			} else {
				webpImages.set(key, false);
				pngThing();
			}
			return;
		}
		if (webpImages.get(key)) {
			webpThing();
		} else {
			pngThing();
		}
